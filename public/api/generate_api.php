...
function requestTrackrShortUrl(string $longUrl, string $shortlinkDomain = ''): ?string
{
    ...
    if (
        ...
        || strtolower((string) $parsedLongUrl['scheme']) !== 'https'
    ) {
        return null;
    }
    ...
    $baseUrl = 'https://' . (string) $parsedLongUrl['host'];
...
function fetchRandomAddonDomain(PDO $pdo, string $subDomain): ?string
{
    $stmt = $pdo->prepare(
        ...
        . 'FROM addondomain WHERE sub_domain = :sub_domain ORDER BY id ASC',
    );
    $stmt->execute(['sub_domain' => $subDomain]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

    return srpPickReadyAddonDomain(
        is_array($rows) ? $rows : [],
        trim(app_env('CF_SERVER_IP', '')),
    );
}
...
try {
    ...
    // (https://<domain>/<base64url-payload>) without the built-in or external
    ...
    if ($noShorten) {
        ...
        // Built-in shortener: store long URL + OG data in DB, return https://host/s-code
